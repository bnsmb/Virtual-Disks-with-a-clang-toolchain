...
cp git git-replace
rm -f git-replay && \
ln -s git git-replay 2>/dev/null || \
cp git git-replay
rm -f git-rerere && \
...
rm -f git-backfill && \
ln -s git git-backfill 2>/dev/null || \
cp git git-backfill

if [ -x ../../bin/scalar ] ; then
  rm -f scalar && \
  ln -s ../../bin/scalar . 2>/dev/null || \
  cp ../../bin/scalar .
fi

if [ -x ../../bin/git ] ; then
  rm -f git && \
  ln -s ../../bin/git . 2>/dev/null || \
  cp ../../git .
fi

if [ -x ../../bin/git-cvsserver ] ; then
  rm -f git-cvsserver && \
  ln -s ../../bin/git-cvsserver . 2>/dev/null || \
  cp ../../bin/git-cvsserver .
fi

if [ -x ../../bin/git-shell ] ; then
  rm -rf git-shell && \
  ln -s ../../bin/git-shell . 2>/dev/null ||  \
  cp ../../bin/git-shell .
fi


rm -f git-gui && \
ln -s ./git-citool git-gui  2>/dev/null || \
cp ./git-citool git-gui
